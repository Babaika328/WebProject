  ...
  <div class="auth-container">
    <div class="auth-card">
      ...
      <div v-if="!codeSent">
        ...
        <input v-model="email" type="email" placeholder="Your email" class="auth-input" required @keyup.enter="sendCode" />
        <button @click="sendCode" :disabled="sending || !email" class="auth-btn primary">
      ...
      <div v-else>
        ...
        <div class="password-wrapper relative mt-6">
          <input v-model="pass.new" :type="showNew ? 'text' : 'password'" placeholder="New password" class="auth-input" required />
        ...
        <div class="password-wrapper relative mt-4">
          <input v-model="pass.confirm" :type="showConfirm ? 'text' : 'password'" placeholder="Confirm new password" class="auth-input" required />
        ...
        <p v-if="passwordError" class="error-text text-center mt-4">{{ passwordError }}</p>

        <button @click="resetPassword" :disabled="resetting || code.length !== 6 || !isPasswordValid" class="auth-btn primary mt-6">
...
const resetPassword = async () => {
  ...
  try {
    ...
    passwordError.value = err.response?.data?.error || 'Invalid or expired code'
    codeDigits.value = ['', '', '', '', '', '']
    nextTick(() => codeInputs.value[0]?.focus())
  } finally {
...
.error-text { @apply text-red-600 font-bold; }
.password-requirements { @apply text-left text-sm mt-3; }
</style>